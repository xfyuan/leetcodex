RSpec.describe 'ResursionSortings' do
  let(:x) { Class.new { extend ResursionSortings } }
  ...
  context 'recursion sortings' do
    it 'with array only have less than 2 elements' do
      expect(x.merge_sorting([])).to eq([])
      expect(x.merge_sorting([5])).to eq([5])

      expect(x.quick_sorting([])).to eq([])
      expect(x.quick_sorting([5])).to eq([5])
    end

    it 'with array have more than 2 elements' do
      expect(x.merge_sorting(nums)).to eq(result)
      expect(x.quick_sorting(nums)).to eq(result)
    end
  end
end
